...
/ milne.c
/
/ A program in C to calculate the area under a curve by using Milne's
/ Method.  This is also called the 4th Degree Open Newton-Cotes Formula.
/
/ Paul Soper
/
/ March 2, 2018
...
int main()
{
  ...
  for (i = 0; i < n; ++i)
  {
    ...
    area = area + ((b-a)/3.0) * (2.0 * f(a + 1.0 * (b - a)/4.0) -
				 1.0 * f(a + 2.0 * (b - a)/4.0) +
				 2.0 * f(a + 3.0 * (b - a)/4.0));
  }
